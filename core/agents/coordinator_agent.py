...
class CoordinatorAgent:
    ...
    def process_message(
        ...
        # 0️⃣ SAFETY CHECK (before anything else)
        safety_result = self.safety.scan_input(text=text, speaker=speaker)
        if not safety_result.get("safe", True):
            reason = safety_result.get(
                "reason",
                "Message blocked due to unsafe or malicious content."
            )
            print(f"🚨 SAFETY BLOCK → {reason}")
            # IMPORTANT: Do NOT create a new session or persist this message.
            return {
                "blocked": True,
                "reason": reason,
                "original_text": text,
